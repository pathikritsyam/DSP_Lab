...
wp = 2*FP/FS; % Pass-band edge frequency (rad/s)
ws = 2*FSB/FS; % Stop-band edge frequency (rad/s)
[n, wn] = buttord(wp, ws, DELTAP, DELTAS);

% Design the analog prototype filter
[b, a] = butter(n, pi*FS*wn, 's');
...
xlabel('Frequency (Hz)');
ylabel('Gain (dB)');
title('Butterworth Low-Pass Filter - Gain Response');
grid on;
